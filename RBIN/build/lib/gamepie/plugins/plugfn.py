...
import tempfile
import subprocess
from gamepie import _gp_log
from urllib.parse import urlparse

protected_plugins = ["GUIasets","Controllers"]
...
def install(input_path):
    ...
    has_gpplug = any(
        f.endswith(".GPplug") and os.path.isfile(os.path.join(input_path, f))
    ...
    if has_gpplug:
        ...
        _gp_log("[plugin warning]: This folder is not a Gamepie plugin "
                "(to make it a plugin, create a .GPplug file in the first level of the folder).")
...
def uninstall(name):
    ...
    gpplug_files = [
        ...
        if f.endswith(".GPplug") and os.path.isfile(os.path.join(target_folder, f))
    ]
